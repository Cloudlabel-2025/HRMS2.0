...
export default function InvoicingPage() {
  ...
  const saveInvoice = async () => {
    ...
    try {
      if (editingId) {
        await api.put(`/api/finance/invoices/${editingId}`, { ...invoiceForm, amount: Number(invoiceForm.amount) });
        notify('Invoice updated');
      } else {
        await api.post('/api/finance/invoices', { ...invoiceForm, amount: Number(invoiceForm.amount) });
        notify('Invoice created');
      }
      setInvoiceForm(EMPTY_INVOICE); setEditingId(null); setShowInvoiceModal(false); load();
    } catch (error) { notify(error.message, 'error'); }
    finally { setSaving(false); }
  };

  const startEdit = (invoice) => {
    setInvoiceForm({ invoiceNo: invoice.invoiceNo || '', client: invoice.client || '', amount: invoice.amount ?? '', issued: invoice.issued || '', due: invoice.due || '', status: invoice.status || 'draft' });
  ...
  const deleteInvoice = async (invoice) => {
    ...
    try {
      await api.delete(`/api/finance/invoices/${invoice._id}`);
      notify('Invoice deleted'); load();
    } catch (error) { notify(error.message, 'error'); }
  };
  ...
  const downloadInvoicePdf = async (invoice) => {
    try {
      ...
      doc.text(`Client: ${invoice.client}`, 14, 34);
      doc.text(`Issued: ${invoice.issued || '—'}`, 14, 40);
      doc.text(`Due: ${invoice.due || '—'}`, 14, 46);
      doc.text(`Status: ${invoice.status}`, 14, 52);
      autoTable(doc, { startY: 60, head: [['Description', 'Amount']], body: [[`Invoice ${invoice.invoiceNo} — ${invoice.client}`, `Rs.${Number(invoice.amount || 0).toLocaleString('en-IN')}`]], styles: { fontSize: 10 }, headStyles: { fillColor: [59, 130, 246] } });
